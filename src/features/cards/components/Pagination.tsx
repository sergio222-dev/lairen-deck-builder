...
export const Pagination = component$<PaginationProps>(({ mobileListDeckRef }) => {
    ...
    const pagesComputed = useComputed$(() => {
        ...
        const start = Math.max(1, f.pagination.page - 2);
        const end   = Math.min(f.pagination.pages, start + 4);
        return Array.from({ length: end - start + 1 }, (_, i) => start + i);
    });

    const isFirstPage = useComputed$(() => f.pagination.page === 1);
    const isLastPage  = useComputed$(() => f.pagination.page === f.pagination.pages);

    // reset scroll position on page change
    useTask$(({ track }) => {
        track(() => f.pagination.page);
                        ...
                        <a
                                ...
                                onClick$={async () => {
                                    if (!isFirstPage.value) {
                                        ...
                                        await f.setPage(f.pagination.page - 1);
                            ...
                            <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                                ...
                                <a
                                        ...
                                        onClick$={async () => {
                                            if (!isLastPage.value) {
                                                appContext.isLoading = true;
                                                await f.setPage(f.pagination.pages)
                                                appContext.isLoading = false;
                                            }
                                        }}
                                        class={`${isLastPage.value ?
