...
import {
    ChipFilter,
    FilterField
}                                                             from "~/components/filterField/FilterField";
import {
    Icon
}                                                             from "~/components/icons/Icon";
import type {
    Filter
}                                                             from "~/models/filters/Filter";
import {
    FILTERS_TYPES
}                                                             from "~/models/filters/Filter";
import {
...
const useSuperTypeFilters = (supertypes: string[]) => {
    const superTypeFilters: Filter[] = supertypes.map(t => {
        return {
            id:         `supertype-${t}`,
            label:      `${t}`,
            value:      t,
...
export const CardFilter = component$<CardFilterProps>(({ mobileListDeckRef }) => {
    // Loaders
    const supertypes = useSuperTypeLoader();
    ...
    const superTypeFilters = useSuperTypeFilters(supertypes.value);
    ...
    const numberOfSuperTypeFilters = useComputed$(() => {
        return c.filters.filter(f => Array.isArray(f.field) && f.field.includes('supertype')).length;
    });
            ...
            {numberOfFilters.value}
                ...
                <dialog ref={refDialog} class="p-4 container max-w-xl">
                  ...
                  ${isFilterInList(f) ? 'bg-primary text-white' : ''}`}
                        ...
                        <Accordion title="Supertypes" quantity={numberOfSuperTypeFilters.value}>
                            <div class="flex flex-wrap gap-2">
                                {superTypeFilters.map(f => (
                                        <button
                                                class={`hover:bg-primary hover:text-white ring-2 ring-primary cursor-pointer px-2 py-1
                  ${isFilterInList(f) ? 'bg-primary text-white' : ''}`}
                                                key={f.id}
